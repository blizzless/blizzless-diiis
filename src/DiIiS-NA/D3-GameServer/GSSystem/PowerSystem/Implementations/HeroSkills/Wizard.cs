...
namespace DiIiS_NA.GameServer.GSSystem.PowerSystem.Implementations
{
	...
	public class WizardSpectralBlade : Skill
	{
		public override IEnumerable<TickTimer> Main()
		{
			...
			if (HasBuff<EnergyTwister.TwisterBuff>(User))   //EnergyTwister -> Storm Chaser
			{
				...
				var proj = new Projectile(this, ActorSno._wizard_tornado_projectile, User.Position);
				proj.Position.Z += 5f;
				proj.RadiusMod = 1.5f;
			...
			blade.OnHit = hitPayload =>
			{
				...
				if (Rune_E > 0 && !HasBuff<BarrierBuff>(User))          //Barrier Blades, buff slot 1
					AddBuff(User, new BarrierBuff(User.Attributes[GameAttributes.Hitpoints_Max_Total] * 0.04f, WaitSeconds(ScriptFormula(9))));
			};
			blade.Apply();
			yield break;
		}
	...
	public class WizardMeteor : PowerScript
	{
		public override IEnumerable<TickTimer> Run()
		{
			...
			for (int n = 0; n < meteorCount; ++n)
			{
				...
				SpawnEffect(
					RuneSelect(
						ActorSno._temp_wizard_meteor_pending,
						ActorSno._wizard_meteor_pending_adddamage,
						ActorSno._temp_wizard_meteor_pending_rune,
						ActorSno._temp_wizard_meteor_pending_frost_rune,
						ActorSno._wizard_meteor_pending_cost,
						ActorSno._temp_wizard_meteor_pending_longer
					),
					impactPos,
					0,
					WaitSeconds(5f)
			...
			foreach (var impactPos in impactPositions)
			{
				...
				if (!(Rune_B > 0))
				{
					var moltenFire = SpawnEffect(
						RuneSelect(
							ActorSno._temp_wizard_meteor_impact,
							ActorSno._wizard_meteor_impact_adddamage,
							ActorSno._temp_wizard_meteor_impact_rune,
							ActorSno._temp_wizard_meteor_impact_frost_rune,
							ActorSno._wizard_meteor_impact_cost, 
							ActorSno._temp_wizard_meteor_impact_longer
						),
						impactPos,
						0,
						poolTime
				...
				if (!(Rune_B > 0))
				{
					SpawnEffect(
						RuneSelect(
							ActorSno._temp_wizard_meteor_afterburn, 
							ActorSno._temp_wizard_meteor_afterburn, 
							ActorSno.__NONE, 
							ActorSno._temp_wizard_meteor_afterburn_frost_rune, 
							ActorSno._wizard_meteor_afterburn_cost, 
							ActorSno._temp_wizard_meteor_afterburn_longer
						),
						impactPos, 
						0,
	...
	public class WizardElectrocute : ChanneledSkill
	{
		...
		public override IEnumerable<TickTimer> Main()
		{
			...
			{
				var proj = new Projectile(this, ActorSno._wizard_electrocute_projectile, User.Position);
				proj.Position.Z += 5f;  // fix height
				proj.RadiusMod = 5f;
				proj.OnCollision = (hit) =>
				{
					hit.PlayEffectGroup(77858);
					WeaponDamage(hit, ScriptFormula(0) * EffectsPerSecond * User.Attributes[GameAttributes.Attacks_Per_Second_Total], DamageType.Lightning);
				};
			...
			else if (Rune_C > 0)        //Arc Lightning
			{
				...
				WeaponDamage(GetEnemiesInArcDirection(User.Position, TargetPosition, ScriptFormula(2), 120f), ScriptFormula(0) * EffectsPerSecond * User.Attributes[GameAttributes.Attacks_Per_Second_Total], DamageType.Lightning);
			}
			else if (Target == null)
			{
			...
			else
			{
				...
				while (targets.Count < ScriptFormula(9) + 1) // original target + bounce 2 times
				{
					...
					if (curTarget.World != null)
					{
						...
						AttackPayload attack = new AttackPayload(this);
						attack.AddWeaponDamage(ScriptFormula(0) * EffectsPerSecond * User.Attributes[GameAttributes.Attacks_Per_Second_Total], DamageType.Lightning);
						...
						attack.OnHit = hitPayload =>
						{
							if (Rune_E > 0 && hitPayload.IsCriticalHit)         //Forked Lightning
							{
								...
								{
									var proj = new Projectile(this, ActorSno._wizard_chargedbolt_projectile, Target.Position);
									proj.Position.Z += 5f;
									proj.OnCollision = (hit) =>
									{
										if (hit == hitPayload.Target) return;
										SpawnEffect(ActorSno._wizard_chargedbolt_groundimpact, new Vector3D(hit.Position.X, hit.Position.Y, hit.Position.Z + 5f)); // impact effect (fix height)
	...
	public class WizardMagicMissile : Skill
	{
		public override IEnumerable<TickTimer> Main()
		{
			...
			if (Rune_B > 0)     //Split
			{
				...
				{
					var proj = new Projectile(this, ActorSno._wizard_magicmissile_projectile, User.Position);
					proj.OnCollision = (hit) =>
					{
			...
			if (Rune_E > 0)     //Seeker
			{
				var projectile = new Projectile(this, ActorSno._wizard_magicmissile_projectile, User.Position);
				...
				else
				{
					...
					for (int i = 0; i < 2; i++)     //yuck... need proper guided projectile
					{
						...
						if (target != null)
						{
							var projectileSeek = new Projectile(this, ActorSno._wizard_magicmissile_projectile, projectile.Position);
							projectile.Destroy();
			...
			var arrow = new Projectile(
				this,
				RuneSelect(
					ActorSno._wizard_magicmissile_projectile,
					ActorSno._wizard_magicmissilerune_damage_projectile,
					ActorSno._wizard_magicmissile_projectile,
					ActorSno._wizard_magicmissilerune_pierce_projectile,
					ActorSno._wizard_magicmissilerune_arcanepower_projectile, 
					ActorSno._wizard_magicmissile_projectile
				), 
				User.Position
			);
			arrow.OnCollision = hit =>
			{
				...
				{
					SpawnEffect(ActorSno._wizard_magicmissilerune_pierce_impact, hit.Position + new Vector3D(0, 0, 5f));
				...
				if (Rune_D > 0)     //Glacial Spike
				{
					...
					SpawnEffect(ActorSno._x1_wizard_magicmissilerune_glacialspike_explosion, hit.Position);
				...
				SpawnEffect(ActorSno._wizard_magicmissile_impact, hit.Position + new Vector3D(0, 0, 5f));
	...
	public class WizardHydra : Skill
	{
		...
		public class HydraBuff : PowerBuff
		{
			...
			{
				if (User.Attributes[GameAttributes.Attacks_Per_Second_Total] > 1f)
					FireRate /= User.Attributes[GameAttributes.Attacks_Per_Second_Total];
			...
			public override bool Apply()
			{
				...
				if (Rune_D > 0)         //Mammoth Hydra has only 1 head
				{
					...
					{
						Hydras[0] = new EffectActor(this, ActorSno._wizard_hydrahead_big, TgtPosition);
						Hydras[0].Timeout = Timeout;
						Hydras[0].Scale = 1.9f;
						Hydras[0].Spawn();

						LavaPools[0] = new EffectActor(this, ActorSno._wizard_hydra_runebig_pool, TgtPosition);
				...
				for (int i = 0; i < spawnPoints.Length; i++)
				{
					if (!User.World.CheckLocationForFlag(spawnPoints[i], DiIiS_NA.Core.MPQ.FileFormats.Scene.NavCellFlags.AllowWalk))
						continue;
					LavaPools[i] = SpawnEffect(
						RuneSelect(
							ActorSno._wizard_hydra_defaultfire_pool,
							ActorSno._wizard_hydra_runefrost_pool,
							ActorSno._wizard_hydra_runelightning_pool,
							ActorSno._x1_wizard_hydra_runefire_pool,
							ActorSno._wizard_hydra_runebig_pool,
					...
					Hydras[i] = SpawnEffect(
						RuneSelect(
							ActorSno._wizard_hydrahead_default_1, 
							ActorSno._wizard_hydrahead_frost_1, 
							ActorSno._wizard_hydrahead_lightning_1, 
							ActorSno._wizard_hydrahead_fire2_1, 
							ActorSno._wizard_hydrahead_big,
							ActorSno._wizard_hydrahead_arcane_1
						),
						spawnPoints[i],
						0,
						Timeout
			...
			public override bool Update()
			{
				...
				if (DotTimer == null || DotTimer.TimedOut)
				{
					...
					foreach (var hydra in Hydras)
					{
						...
						{
							float castAngle = MovementHelpers.GetFacingAngle(hydra.Position, target.Position);
							var firewall = SpawnEffect(ActorSno._wizard_hydra_firewall_proxy, hydra.Position, castAngle, WaitSeconds(FireRate));
						...
						var proj = new Projectile(
							this,
							RuneSelect(
								ActorSno._wizard_hydra_defaultfire_projectile,
								ActorSno._wizard_hydra_frost_projectile,
								ActorSno.__NONE,
								ActorSno._wizard_hydra_defaultfire_projectile,
								ActorSno.__NONE,
								ActorSno._wizard_hydra_runearcane_projectile
							), 
							hydra.Position
						);
						proj.Position.Z += 5f;  // fix height
						proj.OnCollision = (hit) =>
						{
							...
							{
								SpawnEffect(ActorSno._x1_wizard_hydra_fireball_impact, hit.Position);
	...
	public class ArcaneOrb : Skill
	{
		...
		public override IEnumerable<TickTimer> Main()
		{
            ...
            var runeActorSno = RuneSelect(
				...
				ActorSno._x1_wizard_arcaneorb_lightning_orb,
				ActorSno.__NONE,
			...
			{
				var proj2 = new Projectile(this, runeActorSno, startPosition);
				...
				proj2.OnUpdate = () =>      //Resolved all SF to optimize this Update
				{
					...
					if (OrbTimer == null || OrbTimer.TimedOut)
					{
						...
						if (Rune_D > 0)     //Scorch
						{
							var firePool = SpawnEffect(ActorSno._x1_wizard_arcaneorb_scorch, proj2.Position, 0, WaitSeconds(5f));
            ...
            var proj = new Projectile(this, runeActorSno, startPosition);
			proj.Position.Z += 5f;
			proj.OnUpdate = () =>
			{
	...
	public class EnergyTwister : Skill
	{
		public override IEnumerable<TickTimer> Main()
		{
			...
			var Twister = new EffectActor(
				this,
				RuneSelect(
					ActorSno._wizard_tornado,
					ActorSno._x1_wizard_tornado_fire,
					ActorSno._wizard_tornado_obsidian,
					ActorSno._wizard_tornado_golden,
					ActorSno._x1_wizard_tornado_damage,
					ActorSno._wizard_tornado // was 210804
				),
				Rune_E > 0 ? TargetPosition : User.Position
			);
	...
	public class WizardDisintegrate : ChanneledSkill
	{
		...
		public override void OnChannelOpen()
		{
			...
			if (Rune_C <= 0)
			{
				_target = SpawnEffect(
					RuneSelect(
						ActorSno._disintegrate_target,
						ActorSno._disintegrate_target,
						ActorSno._disintegraterune_fatbeam_target,
						ActorSno.__NONE,
						ActorSno._disintegrate_target,
						ActorSno._disintegraterune_death_target
					),
					TargetPosition,
					0,
					WaitInfinite()
		...
		public override IEnumerable<TickTimer> Main()
		{
			...
			if (Rune_C > 0)         //Entropy
			{
				...
				WeaponDamage(GetEnemiesInArcDirection(User.Position, TargetPosition, 20f, 120f), ScriptFormula(1) * ScriptFormula(2) * EffectsPerSecond * User.Attributes[GameAttributes.Attacks_Per_Second_Total], DamageType.Arcane);
			}
			else
				foreach (Actor actor in GetEnemiesInRadius(User.Position, BeamLength + 10f).Actors)
				{
					if (PowerMath.PointInBeam(actor.Position, User.Position, TargetPosition, (Rune_B > 0 ? 6f : 3f)))
					{
						...
						ray.AddWeaponDamage(ScriptFormula(0) * EffectsPerSecond * User.Attributes[GameAttributes.Attacks_Per_Second_Total], DamageType.Arcane);
		...
		public override IEnumerable<TickTimer> Run()
		{
			...
			if (Rune_A > 0)                     //Impactful Wave
			{
				...
				foreach (var proj in projectiles)
				{
					...
					var newProj = new Projectile(this, proj.SNO, proj.Position);
	...
	public class ExplosiveBlast : Skill
	{
		public override IEnumerable<TickTimer> Main()
		{
			...
			{
				SpawnEffect(ActorSno._wizard_explosiveblast_emitter, blastspot);
				AttackPayload attack = new AttackPayload(this);
		...
		IEnumerable<TickTimer> _NoRune()        //Short Fuse, Obliterate, Unleashed
		{
			SpawnEffect(
				RuneSelect(
					ActorSno._wizard_explosiveblast_emitter,
					ActorSno._wizard_explosiveblast_emitter,
					ActorSno._wizard_explosiveblast_indigo_emitter,
					ActorSno.__NONE,
					ActorSno._wizard_explosiveblast_golden_emitter,
					ActorSno.__NONE
				),
				User.Position
			);
		...
		IEnumerable<TickTimer> _RuneE()     //Chain Reaction
		{
			...
			{
				SpawnEffect(ActorSno._wizard_explosiveblast_emitter, User.Position);
				AttackPayload attack = new AttackPayload(this);
				attack.Targets = GetEnemiesInRadius(User.Position, ScriptFormula(2));
	...
	public class WizardArcaneTorrent : ChanneledSkill
	{
		...
		public override void OnChannelOpen()
		{
			...
			_targetProxy = SpawnEffect(
				RuneSelect(
					ActorSno._arcanetorrent_target, 
					ActorSno._arcanetorrent_target_crimson, 
		...
		public override IEnumerable<TickTimer> Main()
		{
			...
			if (Rune_C > 0)         //Arcane Mines
			{
				...
				var mine = new EffectActor(this, ActorSno._wizard_arcanetorrent_runec_proxy, TargetPosition);
				...
				mine.OnUpdate = () =>
				{
					...
					if (targets.Actors.Count > 0 && targets != null)
					{
						...
						mine_attack.AddWeaponDamage(ScriptFormula(29) * EffectsPerSecond * User.Attributes[GameAttributes.Attacks_Per_Second_Total], DamageType.Arcane);
			...
			attack.AddWeaponDamage((Rune_E > 0 ? ScriptFormula(17) : ScriptFormula(0)) * EffectsPerSecond * User.Attributes[GameAttributes.Attacks_Per_Second_Total], DamageType.Arcane);
			attack.OnHit = hitPayload =>
			{
				...
				if (Rune_B > 0 && Rand.NextDouble() < ScriptFormula(12))        //Cascade
				{
					...
					proj.OnCollision = (hit) =>
					{
						...
						WeaponDamage(GetEnemiesInRadius(hit.Position, 5f), ScriptFormula(13) * EffectsPerSecond * User.Attributes[GameAttributes.Attacks_Per_Second_Total], DamageType.Arcane);
						proj.Destroy();
					};
	...
	public class WizardFrostNova : PowerScript
	{
		public override IEnumerable<TickTimer> Run()
		{
			...
			var runeActorSno = RuneSelect(
				ActorSno._g_frostnova_emitter,
				ActorSno._g_frostnova_emitter_crimson_adddamage,
				ActorSno._g_frostnova_emitter_indigo_minifrostnovas,
				ActorSno._wizard_frostnovarune_coldcircle,
				ActorSno._g_frostnova_emitter_golden_reducecooldown,
				ActorSno._g_frostnova_emitter_alabaster_unfreeze
			);
			if (Rune_C > 0)         //Frozen Mist
			{
                var frozenMist = SpawnEffect(runeActorSno, User.Position, 0, WaitSeconds(ScriptFormula(9)));
			...
			SpawnEffect(runeActorSno, User.Position);
		...
		public class ShatterDebuff : PowerBuff
		{
			...
			public override void OnPayload(Payload payload)
			{
				if (payload is DeathPayload && payload.Target == Target)
					...
					{
						SpawnEffect(ActorSno._g_frostnova_emitter_indigo_minifrostnovas, payload.Target.Position);
						AttackPayload nova = new AttackPayload(this);
		...
		public class DeepFreezeChCBuff : PowerBuff
		{
			...
			public override bool Apply()
			{
				...
				Target.Attributes[GameAttributes.Weapon_Crit_Chance] += Percentage;
				Target.Attributes.BroadcastChangedIfRevealed();
				return true;
			}
	...
	public class WizardBlizzard : PowerScript
	{
		public override IEnumerable<TickTimer> Run()
		{
			...
			var blizzard = SpawnEffect(ActorSno._wizard_blizzard, blizzPoint.Position, 0, WaitSeconds(ScriptFormula(4) + User.Attributes[GameAttributes.Power_Duration_Increase, 30680]));
	...
	public class WizardRayOfFrost : ChanneledSkill
	{
		...
		public override void OnChannelOpen()
		{
			...
			{
				_beamEnd = SpawnEffect(ActorSno._wizard_rayoffrost_target, User.Position, 0, WaitInfinite());
		...
		public override IEnumerable<TickTimer> Main()
		{
			...
			else
			{
				...
				if (Rune_E > 0)         //Black Ice
				{
					...
					{
						var icepool = SpawnEffect(ActorSno._wizard_rayoffrostrune_damagepools_proxy, death.Target.Position, 0, WaitSeconds(ScriptFormula(8)));
	...
	public class WizardTeleport : PowerScript
	{
		public override IEnumerable<TickTimer> Run()
		{
			...
			UsePrimaryResource(EvalTag(PowerKeys.ResourceCost));
			if (User is Player && User.Attributes[GameAttributes.Hitpoints_Cur] < (User.Attributes[GameAttributes.Hitpoints_Max_Total] * 0.05f))
			...
			if (Rune_B > 0)     //Fracture
			{
				...
				for (int i = 0; i < maxImages; i++)
				{
					...
					Image.Attributes[GameAttributes.Untargetable] = true;
				...
				foreach (Actor Image in Images)
				{
					...
					Image.Attributes[GameAttributes.Untargetable] = false;
		...
		class TeleDmgReductionBuff : PowerBuff
		{
			...
			public override bool Apply()
			{
				...
				Target.Attributes[GameAttributes.Damage_Absorb_Percent] += ScriptFormula(14);
				Target.Attributes.BroadcastChangedIfRevealed();
				return true;
			}
	...
	public class IceArmor : Skill
	{
		...
		public class IceArmorBuff : PowerBuff
		{
			...
			public override void OnPayload(Payload payload)
			{
				if (payload.Target == Target && payload is HitPayload)
				{
					...
					if (!payload.Context.User.Attributes[GameAttributes.Freeze_Immune])
					{
		...
		class BonusStackEffect : PowerBuff
		{
			...
			public override bool Apply()
			{
				...
				User.Attributes[GameAttributes.Armor_Bonus_Percent] += ScriptFormula(26);
				User.Attributes.BroadcastChangedIfRevealed();
				return true;
			}
			...
			{
				base.Remove();
				User.Attributes[GameAttributes.Armor_Bonus_Percent] -= StackCount * ScriptFormula(26);
				User.Attributes.BroadcastChangedIfRevealed();
			}
		}
		//Rune_C Frozen Storm
	...
	public class WizardShockPulse : Skill
	{
		public override IEnumerable<TickTimer> Main()
		{
            ...
            var count = (Rune_B > 0 || Rune_C > 0) ? 1 : 3;
			var runeActorSno = RuneSelect(
				ActorSno._wizard_chargedbolt_projectile,
				ActorSno._wizard_chargedboltrune_crimson_projectile,
				ActorSno._wizard_chargedboltrune_indigo_projectile,
				ActorSno._wizard_shockpulse_lightningball,
				ActorSno._wizard_chargedboltrune_golden_projectile,
				ActorSno._wizard_chargedboltrune_alabaster_projectile
			);
			for (int n = 0; n < count; ++n)
			{
				...
				proj.OnCollision = (hit) =>
				{
					if (Rune_B <= 0 && Rune_C <= 0)
						SpawnEffect(runeActorSno, new Vector3D(hit.Position.X, hit.Position.Y, hit.Position.Z + 5f)); // impact effect (fix height)
	...
	public class StormArmor : Skill
	{
		...
		class GoldenBuff : PowerBuff        //Power of the Storm
		{
			...
			public override bool Apply()
			{
				...
				User.Attributes[GameAttributes.Resource_Cost_Reduction_Amount] += (int)ScriptFormula(7);
				User.Attributes.BroadcastChangedIfRevealed();
	...
	public class DiamondSkin : Skill
	{
		...
		class DiamondSkinBuff : PowerBuff
		{
			...
			public override void Init()
			{
				...
				HPTreshold = User.Attributes[GameAttributes.Hitpoints_Max_Total] * (Rune_C > 0 ? 0.6f : 0.3f);
			}
			...
			public override bool Apply()
			{
				...
				if (Rune_D > 0)     //Prism
					User.Attributes[GameAttributes.Resource_Cost_Reduction_Amount] += (int)ScriptFormula(4);

				User.Attributes[GameAttributes.Has_Look_Override, 0x061F7489] = true;//0x061F7489;
				User.Attributes.BroadcastChangedIfRevealed();
				return true;
			}
			...
			public override void Remove()
			{
				...
				User.Attributes[GameAttributes.Has_Look_Override, 0x061F7489] = false;
	...
	public class SlowTime : Skill
	{
		public override IEnumerable<TickTimer> Main()
		{
			...
			//No Resouce Cost
			TargetPosition = PowerMath.TranslateDirection2D(User.Position, TargetPosition, User.Position, Math.Min(PowerMath.Distance2D(User.Position, TargetPosition), 50f));
			var TimeWarp = new EffectActor(
				this,
				RuneSelect(
					ActorSno._wizard_slowtime_proxy,
					ActorSno._wizard_slowtimerune_damage_proxy,
					ActorSno._wizard_slowtimerune_linger_proxy,
					ActorSno._wizard_slowtimerune_small_proxy,
					ActorSno._wizard_slowtimerune_cost_proxy,
					ActorSno._wizard_slowtimerune_attackspeed_proxy
				),
				Rune_D > 0 ? TargetPosition : User.Position
			);
			...
			TimeWarp.OnUpdate = () =>
			{
				...
				foreach (Projectile actor in projectilesAround)
				{
					...
					{
						actor.Slowed = false;
						actor.Attributes[GameAttributes.Projectile_Speed] *= 10f;
					}
				}
		...
		class AttackDamageBuff : PowerBuff
		{
			...
			public override bool Apply()
			{
				if (!base.Apply())
					return false;
				Target.Attributes[GameAttributes.Armor_Bonus_Percent] -= ScriptFormula(8);
				return true;
			}
			public override void Remove()
			{
		...
		public class EnergyArmorBuff : PowerBuff
		{
			...
			public override bool Apply()
			{
				...
				if (Rune_A > 0)     //Prismatic Armor
					Target.Attributes[GameAttributes.Resistance_Percent_All] += ScriptFormula(4);

				if (Rune_B > 0)     //Energy Tap
					Target.Attributes[GameAttributes.Resource_Max_Bonus, 1] += ScriptFormula(6);
				else
					Target.Attributes[GameAttributes.Resource_Max_Bonus, 1] -= ScriptFormula(2);

				if (Rune_E > 0)     //Pinpoint Barrier
					Target.Attributes[GameAttributes.Weapon_Crit_Chance] += ScriptFormula(13);

				Target.Attributes[GameAttributes.Armor_Bonus_Percent] += ScriptFormula(1);
				Target.Attributes.BroadcastChangedIfRevealed();
			...
			public override void OnPayload(Payload payload)
			{
				if (payload.Target == Target && payload is HitPayload)
				{
					if (Rune_C > 0)     //Force Armor
						(payload as HitPayload).TotalDamage = Math.Min((payload as HitPayload).TotalDamage, Target.Attributes[GameAttributes.Hitpoints_Max_Total] * ScriptFormula(8));
	...
	public class MagicWeapon : Skill
	{
		...
		class MagicWeaponBuff : PowerBuff
		{
			...
			public override bool Apply()
			{
				...
				User.Attributes[GameAttributes.Damage_Weapon_Percent_Bonus] += ScriptFormula(14);
				User.Attributes[GameAttributes.Damage_Percent_All_From_Skills] += ScriptFormula(14);
				User.Attributes.BroadcastChangedIfRevealed();
					...
					(payload as HitPayload).IsWeaponDamage)
				{
					...
					if (Rune_E > 0 && !HasBuff<DeflectionBuff>(User))       //Deflection
						if (Rand.NextDouble() < lastAttack.Context.GetProcCoefficient())
							AddBuff(User, new DeflectionBuff(User.Attributes[GameAttributes.Hitpoints_Max_Total] * 0.04f, WaitSeconds(ScriptFormula(13))));
				}
			}
		}
	...
	public class Archon : Skill
	{
		...
		public class ArchonBuff : PowerBuff
		{
			...
			public override bool Apply()
			{
				...
				Target.Attributes[GameAttributes.Resistance_Percent_All] += ScriptFormula(4);
				Target.Attributes[GameAttributes.Amplify_Damage_Percent] += ScriptFormula(9);
				Target.Attributes[GameAttributes.Damage_Percent_All_From_Skills] += ScriptFormula(9);

				Target.Attributes[GameAttributes.Skill_Override, 0] = 135166;
				Target.Attributes[GameAttributes.Skill_Override, 1] = 135238;
				Target.Attributes[GameAttributes.Skill_Override, 2] = 167355;
				Target.Attributes[GameAttributes.Skill_Override, 3] = Rune_B > 0 ? 135663 : -1;
				Target.Attributes[GameAttributes.Skill_Override, 4] = Rune_C > 0 ? 167648 : -1;
				Target.Attributes[GameAttributes.Skill_Override, 5] = 166616;
				Target.Attributes[GameAttributes.Skill_Override_Active] = true;
				Target.Attributes.BroadcastChangedIfRevealed();
			...
			public override void OnPayload(Payload payload)
			{
				if (payload.Context.User == Target && payload is DeathPayload)
				{
					...
					Target.Attributes[GameAttributes.Amplify_Damage_Percent] += 0.06f;
					Target.Attributes[GameAttributes.Damage_Percent_All_From_Skills] += 0.06f;
					Target.Attributes.BroadcastChangedIfRevealed();
				}
			}
			...
			public override void Remove()
			{
				...
				{
					Target.Attributes[GameAttributes.Amplify_Damage_Percent] -= KillStreakBonus;
					Target.Attributes[GameAttributes.Damage_Percent_All_From_Skills] -= KillStreakBonus;
					KillStreakBonus = 0f;
				}
				Target.Attributes[GameAttributes.Skill_Override_Active] = false;
				Target.Attributes.BroadcastChangedIfRevealed();
			}
		}
	}
	...
	public class ArchonDisintegrationWave : ChanneledSkill
	{
		...
		{
			EffectsPerSecond = 0.3f;
			DamageMult = ScriptFormula(0) * EffectsPerSecond * User.Attributes[GameAttributes.Attacks_Per_Second_Total];
			_calcTargetPosition();
	...
	public class Familiar : Skill
	{
		...
		class FamiliarBuff : PowerBuff
		{
			...
			public override bool Apply()
			{
				...
				{
					Target.Attributes[GameAttributes.Amplify_Damage_Percent] += ScriptFormula(20);
					Target.Attributes[GameAttributes.Damage_Percent_All_From_Skills] += ScriptFormula(20);
					Target.Attributes.BroadcastChangedIfRevealed();
				}
				if (Rune_D > 0)     //Arcanot
				{
					Target.Attributes[GameAttributes.Resource_Regen_Per_Second, 1] += ScriptFormula(32);
					Target.Attributes.BroadcastChangedIfRevealed();
				}
			...
			public override void OnPayload(Payload payload)
			{
				...
				if (payload.Target == Target && payload is HitPayload && (payload as HitPayload).IsWeaponDamage)
					if (Rune_E > 0 && Target.Attributes[GameAttributes.Hitpoints_Cur] < Target.Attributes[GameAttributes.Hitpoints_Max_Total] * ScriptFormula(35))
				...
				if (_damageTimer == null || _damageTimer.TimedOut)
				{
					...
					if (closest_target != null)
					{
						var proj = new Projectile(
							this,
							RuneSelect(
								ActorSno._wizard_familiar_projectile,
								ActorSno._wizard_familiarrune_damage_projectile,
								ActorSno._wizard_familiarrune_aoe_projectile,
								ActorSno._wizard_familiarrune_pierce_projectile,
								ActorSno._wizard_familiar_projectile,
								ActorSno._wizard_familiarrune_absorb_projectile
							),
							Target.Position
						);
						proj.Position.Z += 5f;
	...
	public class WizardBlackHole : PowerScript
	{
		public override IEnumerable<TickTimer> Run()
		{
			...
			SpawnEffect(
				RuneSelect(
					ActorSno._x1_wizard_blackhole_sphere,
					ActorSno._x1_wizard_blackhole_supermassive_sphere,
					ActorSno._x1_wizard_blackhole_eventhorizon_sphere,
					ActorSno._x1_wizard_blackhole_cosmicgate_sphere,
					ActorSno._x1_wizard_blackhole_spellsteal_sphere,
					ActorSno._x1_wizard_blackhole_absolutezero_sphere
				),
				TargetPosition + new Vector3D(0f, 0f, 5f),
				0,
				WaitSeconds(ScriptFormula(0))
			);

			var wormHole = SpawnEffect(
				RuneSelect(
					ActorSno._x1_wizard_blackhole_proxy,
					ActorSno._x1_wizard_blackhole_supermassive_proxy,
					...
					ActorSno._x1_wizard_blackhole_absolutezero_proxy
				),
				TargetPosition,
				0,
				WaitSeconds(ScriptFormula(0))
			...
			wormHole.OnTimeout = () =>
			{
				if (Rune_C > 0)     //Blazar
				{
					SpawnEffect(ActorSno._x1_wizard_blackhole_cosmicgate_impactsphere, wormHole.Position, 0f, WaitSeconds(1f));
