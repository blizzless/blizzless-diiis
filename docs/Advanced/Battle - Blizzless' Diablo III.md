# Battle | Blizzless D3 - Combat & Brain System Reference
...
6. **`UpdateTarget()`** — priority order is:
   7. `PriorityTarget` (scripted boss focus).
   8. `AttackedBy` (retaliate against whoever hit you last).
   9. Nearest valid target in `DEFAULT_SEARCH_RANGE = 50`.
10. **`IsValidCombatTarget()`** — filters out dead/hidden actors, ghost
